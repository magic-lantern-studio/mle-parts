...
public class Mle2dSet extends MleSet implements I2dSet
{
    ...
    /** The <code>Image</code> to render into. */
    protected MleBufferedImage m_imageBuffer = null;
    ...
    /**
     * The <b>m_headRole</b> is the first Role drawn.  This makes the
     ...
     * The default constructor.
     * <p>
     * The default position is set to (0,0). The default size is set to 640x480.
     * </p>
     */
    public Mle2dSet()
    {
     ...
     * 
     * @param id The task identifier for the render callback.
     */
    ...
    public synchronized boolean handleResizeEvent(Mle2dStage theStage)
    {
	    ...
	    MleBufferedImage stageBuffer = theStage.getPixelBuffer();
	    
	    // Copy the Stage's properties.
	    String[] stageProps = stageBuffer.getPropertyNames();
	    Hashtable properties = null;
	    if (stageProps != null)
	    {
	        properties = new Hashtable();
		    for (int i = 0; i < stageProps.length; i++)
		    {
		        properties.put(stageProps[i],stageBuffer.getProperty(stageProps[i]));
		    }
	    }
	    ...
	    this.m_imageBuffer = new MleBufferedImage(stageBuffer.getColorModel(),
	        stageBuffer.getRaster(), stageBuffer.isAlphaPremultiplied(), properties);
