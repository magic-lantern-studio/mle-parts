...
// Import Magic Lnatern Parts classes.
...
public class Mle2dSetRenderer extends MleRenderEngine
{
    ...
    public void run()
    {
    	...
    	{
            // Clear buffers.
            Graphics offG = theStage.getPixelBuffer().getGraphics();
            offG.setColor(theStage.getBackgroundColor());
            offG.fillRect(
                dp.m_min_x - xoffset,
                dp.m_min_y - yoffset,
                dp.m_max_x - dp.m_min_x + 1,
                dp.m_max_y - dp.m_min_y + 1);
        }
        ...
        MleBufferedImage pixelBuffer = theStage.getPixelBuffer();

        while (nextRole != null)
    	{
    		nextRole.draw(pixelBuffer);
